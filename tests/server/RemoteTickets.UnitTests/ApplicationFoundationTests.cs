...
public sealed class ApplicationFoundationTests
{
    ...
    {
        var request = new CreateTenantRequest("tenant", "database", "Server=.;Database=database", "admin@example.com", "Password1!");
    ...
    public void Master_database_validator_should_accept_valid_connection_and_reject_invalid_values()
    {
        ...
        var valid = new MasterDatabaseSetupCommand(new MasterDatabaseSetupRequest("Server=localhost;Database=master;Integrated Security=True", 30));
    ...
    public async Task Tenant_handlers_should_delegate_all_operations()
    {
        ...
        (await handlers.Handle(new ReceiveContext<CreateTenantCommand>(new("tenant", "db", "Server=.;Database=db", "admin@example.com", "Password1!")), cancellationToken)).Id.Should().Be("tenant");
    ...
    private sealed class FakeTenantService : ITenantManagementService
    {
        public Task<TenantResponse?> GetAsync(string tenantId, CancellationToken cancellationToken)
            => Task.FromResult<TenantResponse?>(new TenantResponse(tenantId, "Tenant", "db", true, true));

        public Task<TenantResponse> CreateAsync(CreateTenantRequest request, CancellationToken cancellationToken)
            => Task.FromResult(new TenantResponse(request.Id, request.Name, request.DatabaseName, true, false));

        public Task<TenantSetupStatusResponse> GetSetupStatusAsync(string tenantId, CancellationToken cancellationToken)
            => Task.FromResult(new TenantSetupStatusResponse(false, true));
